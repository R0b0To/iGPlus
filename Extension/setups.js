...
async function getDrivers() {
  ...
  const { parseAttributes } = await import(chrome.runtime.getURL('/driver/driverHelpers.js'));
  chrome.storage.local.get('script',async function(d){
    ...
    const driver1 = await fetchDriverInfo(drivers[0].attributes[1].value);
    const h1 = parseAttributes(driver1).sHeight;
    ...
    if (drivers.length > 2) {
      const driver2 = await fetchDriverInfo(drivers[1].attributes[1].value);
      const h2 = parseAttributes(driver2).sHeight;
