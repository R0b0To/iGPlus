...
{
  const {hashCode} = await import(chrome.runtime.getURL('scripts/strategy/utility.js'));
  const code = CONFIG.track.code;
...
{
  const {simulateClick} = await import(chrome.runtime.getURL('scripts/strategy/utility.js'));
