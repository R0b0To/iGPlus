  ...
  try {
    ...
    if (document.getElementById('igpXvars') == null) {
      if (league_info != false) {
        ...
        //eventAdded is a placeholder for knowing if the eventliter is already present
...
//observe the fuel change in the dialog for tyre/fuel selection
    var fuelChangeObserver = new MutationObserver(function (mutations) {
      mutations.forEach(mut => {
        ...
        document.getElementById('realfuel').textContent = (parseFloat(fuel_el.textContent) / ((CAR_ECONOMY.fuel + pushToAdd) * TRACK_INFO.length)).toFixed(2);
      });
    });
    ...
    if (document.getElementById('realfuel') == null) {
      ...
      real.textContent = (parseFloat(fuel_el.textContent) / ((CAR_ECONOMY.fuel + pushToAdd) * TRACK_INFO.length)).toFixed(2);
